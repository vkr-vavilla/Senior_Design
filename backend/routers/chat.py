        ...
        try:
            if provider == "gemini":
                ...
                stream = client.chat.completions.create(
                    ...
                    # Stop the model bleeding into the next ChatML turn (leaking "user"
                    # + a hallucinated reply). 151644/151645 are Qwen's <|im_start|>/<|im_end|>.
                    stop=["<|im_start|>", "<|im_end|>"],
                    extra_body={"stop_token_ids": [151644, 151645]},
                )
                for chunk in stream:
