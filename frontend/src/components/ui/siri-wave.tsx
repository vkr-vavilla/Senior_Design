 ...
 * external WebGL library. The shaders are ported from the original
 * `siriWaveCore` / `siriFluidDotsCore` sources, with one addition: a `uAmp`
 * uniform (0 idle, 1 loud) that the "wave" variant uses to scale amplitude,
 * glow intensity, and drift speed — driven every frame by `getAmplitude`,
 * so the wave visibly reacts to whatever audio is currently playing.
 */
...
// Alpha is derived from brightness, not fixed at 1 — near-black background
// pixels become transparent so the wave sits directly on the page's own
// background instead of a black card. Requires the WebGL context to be
// created with {alpha:true, premultipliedAlpha:false} (see SiriWave below),
// so straight (non-premultiplied) alpha composites correctly over the page.
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // Slowed ~2x from real time so the idle wave reads as a calm, unhurried
    // drift rather than a fast mechanical loop.
    float t   = iTime * 0.45;
    // Voice amplitude (0..1), smoothed on the JS side. ampBoost never bottoms
    // out at 0 so the wave stays gently alive between words instead of going
    // flat; it swells toward ~1.5x on louder audio.
    float ampBoost = mix(0.42, 1.5, clamp(uAmp, 0.0, 1.0));
    // Each envelope sums three sines at incommensurate (non-multiple)
    // frequencies instead of a two-factor product — a product of two sines
    // is itself periodic on a short beat cycle and reads as a repeating
    // loop; three unrelated frequencies take a very long time to realign,
    // so the combined motion looks organic/non-repeating at viewing scale.
    float low  = clamp(0.5 + 0.22*sin(t*0.8+0.4)  + 0.16*sin(t*0.37+2.1) + 0.12*sin(t*1.31+5.0), 0.0, 1.0);
    float mid  = clamp(0.5 + 0.20*sin(t*0.53+1.3) + 0.15*sin(t*1.7+3.4)  + 0.10*sin(t*0.29+0.7), 0.0, 1.0);
    float high = clamp(0.5 + 0.18*sin(t*0.71+2.6) + 0.13*sin(t*1.13+5.6) + 0.09*sin(t*0.19+1.9), 0.0, 1.0);

    float res   = clamp(RESOLVED, 0.0, 1.0);
    // No modulo wraparound (the old mod(t, 20*PI) caused a visible snap every
    // ~63s of real time when it reset) — a plain running drift stays smooth
    // for the length of any interview session.
    float drift = t * SPEED * mix(0.8, 1.3, clamp(uAmp, 0.0, 1.0));

    float xN  = p.x / max(aspect, 1.0);
    float env = cos(PI*0.5 * min(abs(0.9*xN), 1.0));
    env *= env;

    float A1    = (AMPLITUDE + 0.01*low*LOW_AMP) * ampBoost;
    ...
    float inten = mix(0.1, 0.01*(INTENSITY + low*LOW_INT), res) * ampBoost;
...
/**
 * Fallback amplitude used whenever no `getAmplitude` is supplied (e.g. a
 * purely decorative placement with no real audio to react to) — a synthetic
 * phrase/pause rhythm so the wave still reads as "talking" instead of a flat
 * idle loop. Pure function of elapsed seconds, no internal state needed.
 */
function syntheticTalkAmplitude(t: number): number {
  // Phrase length wobbles slowly instead of repeating on a strict metronome,
  // mimicking how real sentences vary in length before a breath.
  const BASE_PERIOD = 4.5
  const period = BASE_PERIOD * (1 + 0.25 * Math.sin(t * 0.07))
  const phase = (t % period) / period
  const TALK_FRACTION = 0.62

  if (phase > TALK_FRACTION) {
    // Pause between phrases — near-silent but not perfectly flat.
    return 0.03 + 0.02 * Math.sin(t * 2.3)
  }

  // Envelope across the talking portion: quicker attack than decay.
  const w = phase / TALK_FRACTION
  const envelope = Math.sin(Math.PI * Math.pow(w, 0.7))
  // Syllable-rate bursts on top, three unrelated frequencies so it doesn't
  // read as a clean repeating pulse.
  const burst =
    0.55 +
    0.22 * Math.sin(t * 9.1) +
    0.14 * Math.sin(t * 13.4 + 1.7) +
    0.09 * Math.sin(t * 20.6 + 3.3)

  return Math.max(0, Math.min(1, envelope * burst))
}
...
export interface SiriWaveProps
  ...
  renderScale?: number
  /**
   * Polled once per animation frame — return 0 (idle) to 1 (loud). Used as a
   * getter rather than a numeric prop so amplitude updates (which happen far
   * faster than React's render rate) don't trigger re-renders; the WebGL
   * loop already runs every frame regardless. Omit this for a purely
   * decorative placement (e.g. a landing page) — a synthetic phrase/pause
   * rhythm (`syntheticTalkAmplitude`) is used instead, so the wave still
   * reads as "talking" rather than sitting at a flat idle amplitude.
   */
  getAmplitude?: () => number
}
...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    if (!canvas) return
    // premultipliedAlpha:false so the straight (non-premultiplied) alpha the
    // fragment shader writes composites correctly over the page's own
    // background — the canvas has no bg color of its own.
    ...
    // May be null if the shader optimizes an unused uniform away (fluid-dots
    // declares but doesn't use it) — gl.uniform1f on a null location is a
    // spec-defined no-op, so this is safe either way.
    ...
    const frame = () => {
      ...
      gl.uniform1f(
        uAmp,
        getAmplitudeRef.current ? getAmplitudeRef.current() : syntheticTalkAmplitude(t)
      )
